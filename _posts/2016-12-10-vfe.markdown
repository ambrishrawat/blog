...
**Uncertainty representation**: All the the believed uncertainties in the mathematical description of a model are stated via functional definitions - prior $$ p(\theta|m) $$, likelihood $$ p(D|\theta,m)$$, noise processes, among other expressions. 
...
Inference and Prediction: The task of inference or learning boils down to squeezing prior uncertainties $$p(m) $$ (often with respect to parameters, latent variables etc.) through the data $$ (D) $$ to posterior uncertainties $$(p(\theta|D, m))$$ (following Bayes’ rule).
