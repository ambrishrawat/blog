...
In high-dimensional spaces and multi-modal exact posteriors, VI often fails. The choice of KL-divergence results can result in an approximate posterior where the the optimal apprxoimationm, $$q_{\lambda^*}(\theta)$$ distributes its mass in counter-intuitively. For instance, it could place a high-mass on regions where the orignal distribution had a low or zero mass. 
